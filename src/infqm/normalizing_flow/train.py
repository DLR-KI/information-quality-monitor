...
def compute_normalization_stats(train_loader):
...
class CouplingLayer(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, mask: torch.Tensor) -> None:
        ...
        Args:
            ...
            hidden_dim: Number of hidden units in the scale and translation networks
            mask: Binary mask tensor to determine which features are transformed
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            inverse: Whether to perform inverse transformation (default: False)
...
class RealNVP(nn.Module):
    ...
    def __init__(
        ...
        Args:
            hidden_dim: Number of hidden units in the coupling layers
            num_layers: Number of coupling layers in the flow
            mode: Whether to compute features from "images" or use pre-extracted ones
        ...
        self.image_quality_dimensions_path = Path.cwd() / "src" / "infqm" / "kpi_num"
    ...
    def compute_features(self, x: torch.Tensor):
        """Compute features from input (images or pre-extracted features).

        Args:
            x: Input tensor (images or features)

        Returns:
            torch.Tensor: Computed features for the input
    ...
    def normalize_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Normalized features in the range [0, 1].
        """
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        Returns:
            z: Transformed latent representation
            log_det_j: Log-determinant of the Jacobian of the transformation.
        """
    ...
    def inverse(self, z: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            z: Latent representation to invert back to input space

        Returns:
            torch.Tensor: Inverted input
        """
        x = z
    ...
    def log_prob(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Input tensor (images or features) for which to compute log probability

        Returns:
            torch.tensor: Log probability of the input under the model.
        """
...
def train_normalizing_flow(dataset_name, mode: str = "features") -> None:
    """Train the normalizing flow model.

    Args:
        dataset_name: Name of the dataset to train on
        mode: Whether to compute features from "images" or use pre-extracted "features".
    """
    ...
    feature_min, feature_max = compute_normalization_stats(train_loader)
    ...
    # Optimizer
    optimizer = optim.Adam(model.parameters(), lr=0.001)  # .001
