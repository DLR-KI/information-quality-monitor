...
"""Logging class for normalizing flow training."""
...
class Register:
    ...
    def __init__(self, file, training_iter) -> None:
        """Initialize the logger and create a directory for the current
        training run.

        Args:
            file: Filename of the current training script (e.g., "train.py")
            training_iter: Total number of training iterations (epochs) for this run
        """
        ...
        pathlib.Path(file_dir / f"network_logs/{mid}").mkdir(
            parents=True, exist_ok=True
        )

        # Save path of created folder
    ...
    def finished(self, network, train_losses, val_losses) -> None:
        ...
        Args:
            network: The trained model (e.g., RealNVP)
            train_losses: List of training losses recorded during training
            val_losses: List of validation losses recorded during training
        """
        ...
        with pathlib.Path(self.path / "losses.json").open("w", encoding="utf-8") as f:
            dump({"train_losses": train_losses, "val_losses": val_losses}, f)
    ...
    def get_path(self) -> pathlib.Path:
        ...
        Returns:
            pathlib.Path: Path to the logging folder for the current training run.
        """
        return self.path

    def copy_training_data(self, cvs_file_path) -> pathlib.Path:
        ...
        Returns:
            pathlib.Path: Path to the copied CSV file in the logs folder.
        """
